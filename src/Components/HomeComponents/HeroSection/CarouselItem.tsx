...
const CarouselItem = ({ title1, title2, description, imageUrl }: { title1: string; title2: string; description: string; imageUrl: string }) => {
  return (
    <div className="px-4 md:px-8 lg:px-16 xl:px-20 2xl:px-32 mt-20">
      <div className="flex flex-col-reverse md:flex-row justify-evenly h-screen items-center ">
        <div className="w-full max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-3xl 2xl:max-w-4xl">
          <div className="flex flex-col justify-center p-4 items-start">
            <h2 className="text-4xl font-extrabold text-primary">{title1}<br />{title2}</h2>
            <p className="mt-5 text-lg text-light w-3/4">{description}</p>
          </div>
        </div>
